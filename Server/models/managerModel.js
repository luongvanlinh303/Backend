...
module.exports = {
  ...
  getDetailBooking: async (bookingname) => {
    try {
      ...
      const bookingguard =
        'SELECT Bookingguard.guard_id, guard.firstname,guard.lastname from Bookingguard INNER JOIN guard ON Bookingguard.guard_id = guard.guard_id WHERE Bookingguard.bookingname = $1';
      const customerinfor =
        'SELECT customer.customer_id, customer.firstname,customer.lastname, users.email from Customer INNER JOIN booking ON booking.customer_id = customer.customer_id INNER JOIN users ON users.users_id = customer.users_id WHERE booking.bookingname = $1';
    ...
    try {
      for (let i = 0; i < listguard.length; i++) {
        ...
        const contentManager =
          'You added Guard with name ' + fullName + ' for booking with company name ' + companyname + ' success ';
  ...
  EditGuardBooking: async (bookingname, listguard) => {
    try {
      ...
      for (let i = 0; i < listguard.length; i++) {
        ...
        const resultGuard = await pool.query(selectGuard);
        console.log('selectGuard', resultGuard);

        const { firstname, lastname } = resultGuard.rows[0];
        const fullName = firstname + ' ' + lastname;
        // console.log('full;nam,e', fullName);
        ...
        const formattedDate = moment(booking_date).format('YYYY-MM-DDTHH:mm:ss');
        const content =
          'Admin deleted Guard' + fullName + ' for your booking with company name ' + companyname + ' success ';
        const createNotiCus = {
          text: 'INSERT INTO notiCus (bookingname,customer_id,type,content,publish_date,manager_id) VALUES ($1, $2, $3, $4, $5,$6) RETURNING bookingname',
        ...
        const contentManager =
          'You deleted Guard with name ' + fullName + ' for booking with company name ' + companyname + ' success ';
        const createNotiManager = {
          text: 'INSERT INTO notimanager (bookingname,customer_id,type,guard_id,content,publish_date,manager_id) VALUES ($1, $2, $3, $4, $5,$6,$7) RETURNING bookingname',
      ...
      for (let i = 0; i < listguard.length; i++) {
        const value = listguard[i];
        // console.log('listguard[i]', listguard[i]);
        const queryAdd = {
          text: 'INSERT INTO bookingguard (bookingname, guard_id) VALUES ($1, $2)',
          values: [bookingname, value],
        };
  ...
  postEnoughGuard: async (bookingname) => {
    try {
      ...
      const formattedDate = moment(booking_date).format('YYYY-MM-DDTHH:mm:ss');
      const content =
        'Currently, the number of Guards is not enough to allocate to booking ' +
        companyname +
        ' You can wait or cancel the booking.';
      const createNotiCus = {
        text: 'INSERT INTO notiCus (bookingname,customer_id,type,content,publish_date,manager_id) VALUES ($1, $2, $3, $4, $5,$6) RETURNING bookingname',
